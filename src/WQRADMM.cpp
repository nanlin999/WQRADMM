...
arma::vec WQRCPP(arma::mat x, arma::vec y, double tau, double rho, double eps, int maxstep, bool intercept){
  ...
  double time = 0, time_prep = 0, time_iter = 0;
  ...
  int iteration = 0, minstep = 50;
  double distance = 1;

  while(((distance>eps)|(iteration<minstep))&&(iteration<maxstep)){
    ...
    betaini = beta, z = zini, uini = u, vini = v;
...
arma::mat WeightCPP(arma::mat x, arma::vec y, arma::vec rep, double tau, arma::vec betahat, double eps, double maxstep, bool intercept, String type){
...
arma::mat paraWQRCPP(arma::mat x, arma::vec y, arma::vec rep, int K, double tau, double rho, double eps, int maxstep, bool intercept){
  ...
  while(((distance>eps)|(iteration<50))&&(iteration<maxstep)){
...
arma::mat paraWeightCPP(arma::mat x, arma::vec y, arma::vec rep, int K, double tau, arma::vec betahat, double eps, double maxstep, bool intercept, String type){
...
Rcpp::List WQRADMMCPP(arma::mat x, arma::vec y, arma::vec rep, double tau, bool intercept, String esttype, String corrtype, double rhoCQR, double rhoWQR, double eps, double epsw, int maxstep, int maxstepw){
  ...
  Rcpp::List final;
  if(esttype == "CQR"){
    result_CQR = WQRCPP(x, y, tau, rhoCQR, eps, maxstep, intercept);
    ...
    result_WQR = WQRCPP(xw, yw, tau, rhoWQR, eps, maxstep, intercept);
    double Tsum = 0;
...
Rcpp::List paraWQRADMMCPP(arma::mat x, arma::vec y, int K, arma::vec rep, double tau, bool intercept, String esttype, String corrtype , double rhoCQR, double rhoWQR, double eps, double epsw, int maxstep, int maxstepw){
  ...
  Rcpp::List final;
  if(esttype == "CQR"){
    pararesult_CQR = paraWQRCPP(x, y, rep, K, tau, rhoCQR, eps, maxstep, intercept);
    ...
    pararesult_WQR = paraWQRCPP(xw, yw, rep, K, tau, rhoWQR, eps, maxstep, intercept);
    double Tsum = 0;
